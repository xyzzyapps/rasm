; ============================================================================
; Readable Assembly - SDL Rectangle Game
; ============================================================================
; A simple game that draws a colored rectangle on screen and moves it with
; arrow keys. Demonstrates SDL integration with readable assembly macros.
;
; Build (Linux):
;   nasm -f elf64 sdl_rectangle.asm -o sdl_rectangle.o
;   gcc sdl_rectangle.o -o sdl_rectangle -lSDL2 -no-pie
;
; Build (Windows with MinGW):
;   nasm -f win64 sdl_rectangle.asm -o sdl_rectangle.o
;   gcc sdl_rectangle.o -o sdl_rectangle.exe -lSDL2
; ============================================================================

%include "readable_macros.nasm"

; Windows x64 requires RIP-relative addressing for data symbols
default rel

; ============================================================================
; EXTERNAL SDL FUNCTIONS (C ABI)
; ============================================================================
extern SDL_Init
extern SDL_CreateWindow
extern SDL_CreateRenderer
extern SDL_SetRenderDrawColor
extern SDL_RenderClear
extern SDL_RenderFillRect
extern SDL_RenderPresent
extern SDL_PollEvent
extern SDL_Delay
extern SDL_Quit
extern SDL_GetKeyboardState
extern SDL_DestroyRenderer
extern SDL_DestroyWindow

extern exit
extern printf

; ============================================================================
; CALLING CONVENTION ABSTRACTION
; ============================================================================
; Windows x64: args in rcx, rdx, r8, r9, plus 32-byte shadow space.
; System V (Linux): args in rdi, rsi, rdx, rcx, r8, r9.
; We define ARG1..ARG4 so the call sites below work on both platforms.
%ifidn __OUTPUT_FORMAT__, win64
    %define ARG1 rcx
    %define ARG2 rdx
    %define ARG3 r8
    %define ARG4 r9
    %define SHADOW 32
%else
    %define ARG1 rdi
    %define ARG2 rsi
    %define ARG3 rdx
    %define ARG4 rcx
    %define SHADOW 0
%endif

; ============================================================================
; CONSTANTS
; ============================================================================
%define WINDOW_WIDTH    800
%define WINDOW_HEIGHT   600
%define RECT_SIZE       50
%define MOVE_SPEED      5
%define BG_RED          30
%define BG_GREEN        30
%define BG_BLUE         30
%define BG_ALPHA        255
%define RECT_RED        0
%define RECT_GREEN      200
%define RECT_BLUE       255
%define RECT_ALPHA      255
%define SDL_INIT_VIDEO  0x00000020
%define SDL_WINDOWPOS_UNDEFINED 0x1FFF0000

; SDL event type values
%define SDL_QUIT        0x100
%define SDL_KEYDOWN     0x300

; ============================================================================
; DATA SECTION
; ============================================================================
section .data
    window_title:      define_byte  "Readable Assembly - SDL Rectangle Game", 0
    error_init:        define_byte  "Failed to initialize SDL: %s", 10, 0
    error_window:      define_byte  "Failed to create window: %s", 10, 0
    error_renderer:    define_byte  "Failed to create renderer: %s", 10, 0
    msg_init_failed:   define_byte  "SDL_Init failed", 0
    msg_window_failed: define_byte  "SDL_CreateWindow failed", 0
    msg_renderer_failed: define_byte "SDL_CreateRenderer failed", 0

    ; SDL_Rect structure (x, y, w, h)
    rect_x:           define_dword 375        ; Start centered: (800-50)/2
    rect_y:           define_dword 275        ; Start centered: (600-50)/2
    rect_w:           define_dword RECT_SIZE
    rect_h:           define_dword RECT_SIZE

    ; Keyboard state pointer
    keyboard_state:   define_qword 0

; ============================================================================
; BSS SECTION
; ============================================================================
section .bss
    sdl_window:       reserve_qword 1
    sdl_renderer:     reserve_qword 1
    event_storage:    reserve_byte 64     ; SDL_Event is 56 bytes, keep 64 for safety
    rect_storage:     reserve_byte 16     ; SDL_Rect size

; ============================================================================
; TEXT SECTION
; ============================================================================
section .text
    global main

; ============================================================================
; MAIN - Entry Point
; ============================================================================
main:
    ; --- Prologue ---
    push_onto_stack base_pointer
    move    base_pointer, stack_pointer

    ; --- Initialize SDL ---
    move    ARG1, SDL_INIT_VIDEO
    sub     stack_pointer, SHADOW
    call_procedure SDL_Init
    add     stack_pointer, SHADOW
    compare accumulator, 0
    jump_if_less init_failed

    ; --- Create Window ---
    move    ARG1, window_title
    move    ARG2, SDL_WINDOWPOS_UNDEFINED
    move    ARG3, SDL_WINDOWPOS_UNDEFINED
    move    ARG4, WINDOW_WIDTH
%ifidn __OUTPUT_FORMAT__, win64
    ; 5th arg (h) at [rsp+32], 6th arg (flags) at [rsp+40]
    sub     stack_pointer, 48
    move    [stack_pointer + 32], dword WINDOW_HEIGHT
    move    [stack_pointer + 40], dword 0
%else
    move    register_r8_32, WINDOW_HEIGHT
    move    register_r9_32, 0
%endif
    call_procedure SDL_CreateWindow
%ifidn __OUTPUT_FORMAT__, win64
    add     stack_pointer, 48
%endif
    compare accumulator, 0
    jump_if_equal window_failed
    move    [sdl_window], accumulator

    ; --- Create Renderer ---
    move    ARG1, [sdl_window]
    move    ARG2, -1            ; -1 = first driver
    move    ARG3, 0             ; flags
    sub     stack_pointer, SHADOW
    call_procedure SDL_CreateRenderer
    add     stack_pointer, SHADOW
    compare accumulator, 0
    jump_if_equal renderer_failed
    move    [sdl_renderer], accumulator

    ; --- Get Keyboard State ---
    move    ARG1, 0             ; NULL
    sub     stack_pointer, SHADOW
    call_procedure SDL_GetKeyboardState
    add     stack_pointer, SHADOW
    move    [keyboard_state], accumulator

    ; ========================================================================
    ; GAME LOOP
    ; ========================================================================
game_loop:
    ; --- Poll for events ---
    move    ARG1, event_storage
    sub     stack_pointer, SHADOW
    call_procedure SDL_PollEvent
    add     stack_pointer, SHADOW
    compare accumulator, 0
    jump_if_not_equal check_events

    ; --- No events: process keyboard input ---
    call_procedure process_input

    ; --- Clear screen ---
    move    ARG1, [sdl_renderer]
    move    ARG2, BG_RED
    move    ARG3, BG_GREEN
    move    ARG4, BG_BLUE
%ifidn __OUTPUT_FORMAT__, win64
    sub     stack_pointer, 48
    move    [stack_pointer + 32], dword BG_ALPHA
%else
    move    register_r8_32, BG_ALPHA
%endif
    call_procedure SDL_SetRenderDrawColor
%ifidn __OUTPUT_FORMAT__, win64
    add     stack_pointer, 48
%endif

    move    ARG1, [sdl_renderer]
    sub     stack_pointer, SHADOW
    call_procedure SDL_RenderClear
    add     stack_pointer, SHADOW

    ; --- Draw rectangle ---
    call_procedure draw_rectangle

    ; --- Present renderer ---
    move    ARG1, [sdl_renderer]
    sub     stack_pointer, SHADOW
    call_procedure SDL_RenderPresent
    add     stack_pointer, SHADOW

    ; --- Frame delay (16ms ~ 60fps) ---
    move    ARG1, 16
    sub     stack_pointer, SHADOW
    call_procedure SDL_Delay
    add     stack_pointer, SHADOW

    ; --- Loop ---
    jump    game_loop

; ============================================================================
; CHECK EVENTS
; ============================================================================
check_events:
    ; Event type is the first 4 bytes of SDL_Event
    move    accumulator_32, [event_storage]
    compare accumulator_32, SDL_QUIT
    jump_if_equal cleanup

    compare accumulator_32, SDL_KEYDOWN
    jump_if_equal check_escape

    jump    game_loop

check_escape:
    ; SDL_KeyboardEvent.keysym.scancode is at offset 16
    move    accumulator_32, [event_storage + 16]
    compare accumulator_32, 41            ; SDL_SCANCODE_ESCAPE
    jump_if_equal cleanup
    jump    game_loop

; ============================================================================
; PROCESS INPUT
; ============================================================================
; Reads the keyboard state array (indexed by SDL scancode) and updates the
; rectangle position. No external calls, so no shadow space needed.
; ============================================================================
process_input:
    ; Prologue
    push_onto_stack base_pointer
    move    base_pointer, stack_pointer
    push_onto_stack base

    move    base, [keyboard_state]

    ; --- Check Up Arrow (scancode 82) ---
    move    accumulator_8, [base + 82]
    compare accumulator_8, 0
    jump_if_equal check_down
    ; Move up
    move    accumulator_32, [rect_y]
    subtract accumulator_32, MOVE_SPEED
    move    [rect_y], accumulator_32

check_down:
    ; --- Check Down Arrow (scancode 81) ---
    move    accumulator_8, [base + 81]
    compare accumulator_8, 0
    jump_if_equal check_left
    ; Move down
    move    accumulator_32, [rect_y]
    add     accumulator_32, MOVE_SPEED
    move    [rect_y], accumulator_32

check_left:
    ; --- Check Left Arrow (scancode 80) ---
    move    accumulator_8, [base + 80]
    compare accumulator_8, 0
    jump_if_equal check_right
    ; Move left
    move    accumulator_32, [rect_x]
    subtract accumulator_32, MOVE_SPEED
    move    [rect_x], accumulator_32

check_right:
    ; --- Check Right Arrow (scancode 79) ---
    move    accumulator_8, [base + 79]
    compare accumulator_8, 0
    jump_if_equal clamp_position
    ; Move right
    move    accumulator_32, [rect_x]
    add     accumulator_32, MOVE_SPEED
    move    [rect_x], accumulator_32

clamp_position:
    ; --- Clamp X position ---
    move    accumulator_32, [rect_x]
    compare accumulator_32, 0
    jump_if_greater_or_equal clamp_x_max
    move    size_dword [rect_x], 0

clamp_x_max:
    move    accumulator_32, [rect_x]
    compare accumulator_32, WINDOW_WIDTH - RECT_SIZE
    jump_if_less_or_equal clamp_y_min
    move    size_dword [rect_x], WINDOW_WIDTH - RECT_SIZE

clamp_y_min:
    ; --- Clamp Y position ---
    move    accumulator_32, [rect_y]
    compare accumulator_32, 0
    jump_if_greater_or_equal clamp_y_max
    move    size_dword [rect_y], 0

clamp_y_max:
    move    accumulator_32, [rect_y]
    compare accumulator_32, WINDOW_HEIGHT - RECT_SIZE
    jump_if_less_or_equal process_done
    move    size_dword [rect_y], WINDOW_HEIGHT - RECT_SIZE

process_done:
    pop_from_stack base
    move    stack_pointer, base_pointer
    pop_from_stack base_pointer
    return_from_procedure

; ============================================================================
; DRAW RECTANGLE
; ============================================================================
draw_rectangle:
    ; Prologue
    push_onto_stack base_pointer
    move    base_pointer, stack_pointer

    ; Copy rect values to rect_storage
    move    accumulator_32, [rect_x]
    move    [rect_storage], accumulator_32
    move    accumulator_32, [rect_y]
    move    [rect_storage + 4], accumulator_32
    move    accumulator_32, [rect_w]
    move    [rect_storage + 8], accumulator_32
    move    accumulator_32, [rect_h]
    move    [rect_storage + 12], accumulator_32

    ; Set draw color to rectangle color
    move    ARG1, [sdl_renderer]
    move    ARG2, RECT_RED
    move    ARG3, RECT_GREEN
    move    ARG4, RECT_BLUE
%ifidn __OUTPUT_FORMAT__, win64
    sub     stack_pointer, 48
    move    [stack_pointer + 32], dword RECT_ALPHA
%else
    move    register_r8_32, RECT_ALPHA
%endif
    call_procedure SDL_SetRenderDrawColor
%ifidn __OUTPUT_FORMAT__, win64
    add     stack_pointer, 48
%endif

    ; Fill rectangle
    move    ARG1, [sdl_renderer]
    move    ARG2, rect_storage
    sub     stack_pointer, SHADOW
    call_procedure SDL_RenderFillRect
    add     stack_pointer, SHADOW

    ; Epilogue
    move    stack_pointer, base_pointer
    pop_from_stack base_pointer
    return_from_procedure

; ============================================================================
; CLEANUP
; ============================================================================
cleanup:
    ; Destroy renderer
    move    ARG1, [sdl_renderer]
    sub     stack_pointer, SHADOW
    call_procedure SDL_DestroyRenderer
    add     stack_pointer, SHADOW

    ; Destroy window
    move    ARG1, [sdl_window]
    sub     stack_pointer, SHADOW
    call_procedure SDL_DestroyWindow
    add     stack_pointer, SHADOW

    ; Quit SDL
    sub     stack_pointer, SHADOW
    call_procedure SDL_Quit
    add     stack_pointer, SHADOW

    ; Exit with code 0
    move    ARG1, 0
    sub     stack_pointer, SHADOW
    call_procedure exit

; ============================================================================
; ERROR HANDLERS
; ============================================================================
init_failed:
    move    ARG1, error_init
    move    ARG2, msg_init_failed
    move    accumulator_8, 0
    sub     stack_pointer, SHADOW
    call_procedure printf
    add     stack_pointer, SHADOW
    move    ARG1, 1
    sub     stack_pointer, SHADOW
    call_procedure exit

window_failed:
    move    ARG1, error_window
    move    ARG2, msg_window_failed
    move    accumulator_8, 0
    sub     stack_pointer, SHADOW
    call_procedure printf
    add     stack_pointer, SHADOW
    sub     stack_pointer, SHADOW
    call_procedure SDL_Quit
    add     stack_pointer, SHADOW
    move    ARG1, 1
    sub     stack_pointer, SHADOW
    call_procedure exit

renderer_failed:
    move    ARG1, error_renderer
    move    ARG2, msg_renderer_failed
    move    accumulator_8, 0
    sub     stack_pointer, SHADOW
    call_procedure printf
    add     stack_pointer, SHADOW
    move    ARG1, [sdl_window]
    sub     stack_pointer, SHADOW
    call_procedure SDL_DestroyWindow
    add     stack_pointer, SHADOW
    sub     stack_pointer, SHADOW
    call_procedure SDL_Quit
    add     stack_pointer, SHADOW
    move    ARG1, 1
    sub     stack_pointer, SHADOW
    call_procedure exit
